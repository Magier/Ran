...
func TestParseEffect_TargetIP(t *testing.T) {
	...
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("sys.ip", target, nil, domain.Procedure{}, args, results...)

	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}

	if len(updatedFacts.New.Entities) != 1 {
		t.Fatalf("Expected 1 entity, got %d", len(updatedFacts.New.Entities))
	}
	pod, ok := updatedFacts.New.Entities[0].(domain.Pod)
	if !ok {
	...
	if len(updatedFacts.Removed.Entities) != 0 {
...
func TestParseEffect_K8sPodList(t *testing.T) {
	...
	args := map[string]string{}
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("k8s.podlist", source, nil, domain.Procedure{}, args, results...)
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}

	if len(updatedFacts.New.Entities) != 2 {
		t.Fatalf("Expected 2 entities, got %d", len(updatedFacts.New.Entities))
	}
	if len(updatedFacts.Removed.Entities) != 0 {
		t.Fatalf("Expected 0 removed entities")
	}
...
func TestParseEffect_K8sDeploymentList(t *testing.T) {
	...
	args := map[string]string{}
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("k8s.secretlist", source, nil, domain.Procedure{}, args, results...)
...
func TestParseEffect_K8sServiceAccountList(t *testing.T) {
	...
	args := map[string]string{}
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("k8s.serviceaccountlist", source, nil, domain.Procedure{}, args, results...)
...
func TestParseEffect_K8sServiceAccount_Created(t *testing.T) {
	...
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("k8s.serviceaccount", source, nil, domain.Procedure{}, args, results...)
	...
	sa, ok := updatedFacts.New.Entities[0].(domain.ServiceAccount)
	if !ok {
...
func TestParseEffect_DeleteK8sServiceAccount(t *testing.T) {
	...
	args := map[string]string{"Name": "my-sa", "Namespace": "ns1"}
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("delete k8s.serviceaccount", source, nil, domain.Procedure{}, args, results...)

	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}

	if len(updatedFacts.Removed.Entities) != 1 {
		t.Fatalf("Expected 1 removed entity, got %d", len(updatedFacts.Removed.Entities))
	}
	sa, ok := updatedFacts.Removed.Entities[0].(domain.ServiceAccount)
	if !ok {
	...
	if len(updatedFacts.New.Entities) != 0 {
...
func TestParseEffect_DeleteK8sPod(t *testing.T) {
	...
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("delete k8s.pod", source, nil, domain.Procedure{}, args, results...)
	...
	pod, ok := updatedFacts.Removed.Entities[0].(domain.Pod)
	if !ok {
...
func TestParseEffect_DeleteK8sDeployment(t *testing.T) {
	...
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("delete k8s.deployment", source, nil, domain.Procedure{}, args, results...)
	...
	dep, ok := updatedFacts.Removed.Entities[0].(domain.Deployment)
	if !ok {
...
func TestParseEffect_UnknownEffect(t *testing.T) {
	...
	args := map[string]string{}
	c := NewCampaign(nil)
	updatedFacts, err := c.ParseEffect("unknown.effect", source, nil, domain.Procedure{}, args, results...)
	...
	if len(updatedFacts.New.Entities) != 0 {
		t.Fatalf("Expected 0 entities, got %d", len(updatedFacts.New.Entities))
	}
	if len(updatedFacts.Removed.Entities) != 0 {
		t.Fatalf("Expected 0 removed entities")
	}
...
func TestParseEffect_NoResults(t *testing.T) {
	...
	args := map[string]string{}
	c := NewCampaign(nil)
	_, err := c.ParseEffect("k8s.podist", source, nil, domain.Procedure{}, args)
