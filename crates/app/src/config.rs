...
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct KubetierConfig {
    /// Optional private/full catalog generated by the KubeTier importer.
    /// The bundled metadata-only catalog is used when omitted.
...
pub fn load(path: Option<PathBuf>) -> Result<Config> {
    ...
    if let Some(catalog) = &mut cfg.kubetier.catalog {
        if catalog.is_relative() {
            *catalog = base_dir.join(&*catalog);
        }
    }
...
mod tests {
    ...
    #[test]
    fn kubetier_catalog_is_resolved_relative_to_config() {
        let temp = tempfile::tempdir().unwrap();
        let config_path = temp.path().join("ran.yaml");
        std::fs::write(
            &config_path,
            "kubetier:\n  catalog: private/kubetier.full.json\n",
        )
        .unwrap();
        let config = load(Some(config_path)).unwrap();
        assert_eq!(
            config.kubetier.catalog,
            Some(temp.path().join("private/kubetier.full.json"))
        );
    }
}
